...
```{r}
simulations <- c(
  "IV250" = loadSimulation(system.file(
    "extdata",
    "Clarithromycin_Chu_1992_iv_250mg.pkml",
    package = "ospsuite.parameteridentification"
  )),
  "PO250" = loadSimulation(system.file(
    "extdata",
    "Clarithromycin_Chu_1993_po_250mg.pkml",
    package = "ospsuite.parameteridentification"
  )),
  "PO250MD" = loadSimulation(system.file(
    "extdata",
    "Clarithromycin_Chu_1993_po_250mg_md.pkml",
    package = "ospsuite.parameteridentification"
  )),
  "PO500" = loadSimulation(system.file(
    "extdata",
    "Clarithromycin_Chu_1993_po_250mg.pkml",
    package = "ospsuite.parameteridentification"
  )),
  "PO500MD" = loadSimulation(system.file(
    "extdata",
    "Clarithromycin_Chu_1993_po_250mg_md.pkml",
    package = "ospsuite.parameteridentification"
  ))
...
parameterInputData <- list(
  ...
  list(
    path = "Neighborhoods|Kidney_pls_Kidney_ur|Clarithromycin|Renal Clearances-fitted|Specific clearance",
    min = 0,
    max = 100,
    start = 10
  ),
...
outputMappings[[1]] <- PIOutputMapping$new(
  quantity = getQuantity(outputPath, container = simulations$IV250)
)
outputMappings[[1]]$addObservedDataSets(
  obsData$Clarithromycin_Profiles.IV250.IV250SD
)

outputMappings[[2]] <- PIOutputMapping$new(
  quantity = getQuantity(outputPath, container = simulations$PO250)
)
outputMappings[[2]]$addObservedDataSets(
  obsData$Clarithromycin_Profiles.PO250.PO500SD
)

outputMappings[[3]] <- PIOutputMapping$new(
  quantity = getQuantity(outputPath, container = simulations$PO250MD)
)
outputMappings[[3]]$addObservedDataSets(
  obsData$Clarithromycin_Profiles.PO250MD.PO250MD
)

outputMappings[[4]] <- PIOutputMapping$new(
  quantity = getQuantity(outputPath, container = simulations$PO500)
)
outputMappings[[4]]$addObservedDataSets(
  obsData$Clarithromycin_Profiles.PO500.PO500SD
)

outputMappings[[5]] <- PIOutputMapping$new(
  quantity = getQuantity(outputPath, container = simulations$PO500MD)
)
outputMappings[[5]]$addObservedDataSets(
  obsData$Clarithromycin_Profiles.PO500MD.PO500MD
)
