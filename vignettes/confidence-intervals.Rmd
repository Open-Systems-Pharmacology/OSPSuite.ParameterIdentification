...
-   **`bootstrap`** generates confidence intervals by repeatedly resampling the
    ...
    **Note**: When using the bootstrap method, if the parameter estimate falls outside 
    the computed confidence interval, which can occur with highly skewed distributions, 
    the package automatically reports a one-sided confidence interval. The conflicting 
    bound is set to `NA`, and the `ciType` field is set to `"one-sided"`. If both 
    bounds conflict, `ciType` is set to `"failed"`. For successfully estimated 
    two-sided intervals, `ciType` is `"two-sided"`.

Under `objectiveType = "mle"` (see `vignette("error-calculation")`), `hessian` and
`PL` are not yet corrected for the likelihood scale: both still read the curvature
of the objective through least-squares formulas, and both warn about this at run
time. `bootstrap` re-optimizes the same objective instead of reading its curvature,
so it remains the reliable choice under `mle`.
...
- For `bootstrap`, begin with a small number of resamples (for example 100-200) 
