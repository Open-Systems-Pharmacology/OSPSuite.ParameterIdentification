...
```{r include=TRUE, eval=FALSE}
profile <- task$calculateProfiles(lower = -10, upper = 10, totalEvaluations = 200)
task$plotOFVProfiles(profile)[[1]]
```
```{r include=TRUE, echo=FALSE, eval=TRUE, fig.width=6, fig.height=4, fig.align='center'}
...
```{r include=TRUE, eval=FALSE}
grid <- task$calculateGrid(totalEvaluations = 1000, setStartingPoint = TRUE)
...
```{r include=TRUE, eval=FALSE}
task$plotOFVProfiles(task$calculateProfiles())
```
```{r include=TRUE, echo=FALSE, eval=TRUE, fig.width=6, fig.height=4, fig.align='center'}
