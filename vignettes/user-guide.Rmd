...
-   Use `$toList()` to export full details, including diagnostics, cost metrics, and residuals.\
    `costDetails` includes items such as `rawSSR`, `weightedSSR`, `rawResiduals`, `weightedResiduals`, `errorWeights`, `userWeights`, `yObserved`, and `ySimulated`:
...
A converged optimum should sit in a locally convex region of the objective function. Inspect this with `calculateOFVProfiles()`, which varies each `PIParameter` independently around the current values while holding the others fixed. The result is a named list of tibbles (one per parameter), each holding the parameter's grid values and the matching `ofv`. Pass it to `plotOFVProfiles()` to visualize:
...
By default the grid covers ±10% (`boundFactor = 0.1`) around the current value at 20 evaluations per parameter. To widen the explored neighborhood or refine the grid, pass `boundFactor` and `totalEvaluations`:
...
A roughly parabolic profile with the minimum near the current value indicates a well-conditioned optimum; a flat or monotonic profile suggests poor identifiability of that parameter.
