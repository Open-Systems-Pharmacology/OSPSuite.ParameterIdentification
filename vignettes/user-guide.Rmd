...
```
```{r include=TRUE, eval=FALSE}
profile <- task$calculateProfiles()
task$plotOFVProfiles(profile)[[1]]
```
```{r include=FALSE, eval=TRUE, fig.width=6, fig.height=4, fig.align='center'}
task$plotOFVProfiles(cachedAciclovirProfile)[[1]]
```
We observe that the tentative lipophilicity value of `r signif(taskResults$par[[1]], 3)` is close to at least a local (and potentially, a global) minimum. By passing explicit parameters to the `calculateProfiles` function, we can check the whole parameter space, at least for 1-dimensional parameter estimation problems: 
```{r include=TRUE, eval=FALSE}
profile <- task$calculateProfiles(lower = -10, upper = 10, totalEvaluations = 100)
task$plotOFVProfiles(profile)[[1]]
```
```{r include=FALSE, eval=TRUE, fig.width=6, fig.height=4, fig.align='center'}
...
In addition to previous diagnostic methods, we can also plot the landscape of the objective function, since the problem is 2-parametric and it is easily visualized with a contour plot. The package includes `$calculateGrid()` and `$plotOFVGrid()` functions for that: 
```{r include=TRUE, eval=FALSE}
grid <- task$calculateGrid(lower = c(3, 0), upper = c(4, 5), totalEvaluations = 2000)
task$plotOFVGrid(grid)
```
```{r include=FALSE, eval=TRUE, fig.width=10, fig.height=6, fig.align='center'}
task$plotOFVGrid(cachedMidazolamGrid)
```
...
The solution found by the BOBYQA algorithm (14.10, 6.24, 1.306e-6 > 4.17) with the default parameters is satisfactory, because the individual profiles are close to the observed data. However, it is sub-optimal, as can be evidenced by non-parabolic OFV profiles: 
```{r include=TRUE, eval=FALSE}
taskResults <- task$run()
task$plotOFVProfiles(task$calculateProfiles())[[1]]
```
```{r include=FALSE, eval=TRUE, fig.width=6, fig.height=4, fig.align='center'}
task$plotOFVProfiles(task$calculateProfiles(par = cachedClarithromycinOptimalPoint))[[1]]
```

If a sub-optimal solution is not enough, we need to adapt the local algorithm settings, use grid search or a global stochastic optimization algorithm, such as differential evolution, to find a good starting approximation or a global minimum. 

### Local algorithm settings

### Exhaustive grid search
Running the grid search with 3000 evaluations produces a better starting point (15.4, 7.69, 0.385 > 32.2). This can be a useful first step when you don't know the optimization landscape of the problem. 
```{r include=TRUE, eval=FALSE}
grid <- task$calculateGrid(totalEvaluations = 3000, setStartingPoint = TRUE)
...
For example, running 1000 iterations of differential optimization produces a result with a better OFV: ???.
