...
General workflow description. The overall workflow is similar to running parameter identification in PK-Sim or MoBi, see the corresponding documnetation chapter <https://docs.open-systems-pharmacology.org/shared-tools-and-example-workflows/parameter-identification>

A PI task is defined by:

-   A set of simulations

    -   You can use multiple simulations in one optimization task. As an example, you can indetify parameter values using data for different dosings.
...
In order to calculate the error of the model, simulation outputs must be linked to observed data. In this framework, one simulation output can be linked to multiple observed data sets. The error will be calculated for each pair of simulated output and the linked data set and added up for the total error. The mapping is done by creating an object of the class `PIOutputMapping`. Each object links a single `Quantity` of a simulation to multiple [`DataSet`](https://www.open-systems-pharmacology.org/OSPSuite-R/reference/DataSet.html) objects.

All data sets (simulated results and obsreved data) within a `PIOutputMapping` can be transformed by applying offsets and scaling factors following the same logic as implemented in the [`DataCombined` class](https://www.open-systems-pharmacology.org/OSPSuite-R/articles/data-combined.html#transformations). Finally, the scaling of the output mapping defines how results are calculated. ADD SHORT DESCRIPTION HERE. LIN: SIM-OBS, log: log(SIM/OBS).

#### Definition of parameters to be identified

Specification of parameters to be idenfied is done by creating objects of the class `PIParameters`. A `PIParameter` describes a single model parameter of a group of parameters that will be optimized together, i.e., all model parameters grouped to a `PIParameters` object will have the same value. A `PIParameter` also defines the start and the minimal and maximal values for the optimization.
...
EXAMPLES FOR THE ABOVE DESCRIBED CASES
...
Create a parameter identification task by creating an object of the class `ParameterIdentification`. The object holds the set of simulations that were used for the creation of the `PIOutputMapping` and `PIParameters`, and the configuration.

Overview - plot the (current) results, run the identification, run grid search, plot OFVprofiles. Describe the theory and link to examples below.
...
The default lower and uppr bounds are 0.1- and 10-fold of the starting parameter value, respectrively, but it is often useful to supply parameter bounds from prior knowledge:
...
```{r include=TRUE, eval=TRUE, fig.width=10, fig.height=6, fig.align='center', warning=FALSE}
task$plotResults()
```
...
```{r include=TRUE, eval=FALSE}
taskResults <- task$run()
plotOFVProfiles(task$calculateOFVProfiles())[[1]]
```

```{r include=TRUE, echo=FALSE, eval=TRUE, fig.width=6, fig.height=4, fig.align='center'}
plotOFVProfiles(cachedClarithromycinSuboptimalProfiles)[[1]]
```
...
```{r include=TRUE, eval=FALSE}
plotOFVProfiles(task$calculateOFVProfiles())
```

```{r include=TRUE, echo=FALSE, eval=TRUE, fig.width=6, fig.height=4, fig.align='center'}
plotOFVProfiles(cachedClarithromycinOptimalProfiles)[[1]]
plotOFVProfiles(cachedClarithromycinOptimalProfiles)[[2]]
plotOFVProfiles(cachedClarithromycinOptimalProfiles)[[3]]
```
