...
```
```{r include=TRUE, eval=TRUE, fig.width=6, fig.height=4, fig.align='center'}
...
```{r include=FALSE, message=FALSE, eval=TRUE}
taskResults <- task$run()
```
...
```{r include=TRUE, eval=TRUE, fig.width=5, fig.height=4, fig.align='center'}
...
Running the grid search with 1000 evaluations produces a better starting point. This can be a useful first step when you don't know the optimization landscape of the problem. 
```{r include=TRUE, eval=TRUE}
grid <- task$calculateGrid(totalEvaluations = 1000)
dplyr::slice_min(grid, ofv, with_ties = FALSE)
```
This starting point can be used for later optimization runs with `BOBYQA` or `DEoptim`. 



For example, running 100 iterations of differential optimization produces a result with a better OFV.
```{r include=TRUE, message=FALSE, eval=TRUE}
