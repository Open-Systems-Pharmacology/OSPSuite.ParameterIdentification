...
piConfiguration$objectiveFunctionOptions <- list(
  objectiveFunctionType = "lsq" # or "m3"
)
```
...
piConfiguration$objectiveFunctionOptions <- list(
  ...
  residualWeightingMethod = "std" # or "mean" or "error"
)
```
...
To apply robust residual calculation, set the `robustResidualCalculation` attribute in `objectFunctionOptions` of the `PIConfiguration` object:

```{r, eval=FALSE}
piConfiguration$objectiveFunctionOptions <- list(
  objectiveFunctionType = "lsq",
  robustResidualCalculation = "huber" # or "bisquare"
)
```
