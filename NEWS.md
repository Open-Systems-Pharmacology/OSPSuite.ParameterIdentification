...
- `PKOutputMapping` adds PK-metric optimization to `ParameterIdentification`: fit one or more parameters to target PK metrics (C_max, AUC_tEnd, etc.) supplied as scalar values or computed from observed `DataSet` objects.
